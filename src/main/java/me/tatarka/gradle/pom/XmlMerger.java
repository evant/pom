...
final class XmlMerger {
    ...
    static void mergePom(Node pomXml, Closure userPom) {
        ...
        userPom.setResolveStrategy(Closure.DELEGATE_FIRST);
    ...
    private static void mergeChild(Node root, Node child) {
        ...
        Node target = findPath((NodeList) root.get((String) child.name()), child);
        if (target != null) {
            if (isLeaf(child)) {
    ...
    private static String nid(Node node) {
        return (String) node.attributes().get(NID_ATTR);
    }
    ...
    private static Node findPath(NodeList list, Node child) {
        ...
        Node firstNidNull = null;
        // look for matching id or any node without id
        for (int i = 0, size = list.size(); i < size; i++) {
            Node node = (Node) list.get(i);
            if (childID.equals(nid(node))) {
                return node;
            }
            if (firstNidNull == null && nid(node) == null) {
                firstNidNull = node;
            }
        }
        return firstNidNull;
    ...
    private static boolean isLeaf(Node node) {
        ...
        return children.isEmpty() || !(children.get(0) instanceof Node);
    }
}
